    ...
    for (int kk = 0; kk < V; kk++){
        if (Component[kk] < 0){
            ...
            while (!Q.empty())
            {
              ...
              {
                if (Component[EdgeList[e]] < 0)
                { 
                  Q.push(EdgeList[e]);
                  Component[EdgeList[e]] = NCC_BFS;
                }
              }
              current = Q.front();
            }
...
void DFS(int v, int *FirstVertex, int V, int *EdgeList,int E, int *Component, int NCC){
    ...
    Component[v] = NCC;
    int comp_index; // points to a vertex
    for (int e = FirstVertex[v]; e < FirstVertex[v+1]; e++)
    {
      comp_index = EdgeList[e];
      if (Component[comp_index] < 0)
      {
        // call DFS on the vertex found unmarked
        DFS(comp_index, FirstVertex, V, EdgeList, E, Component, NCC);
      }
    }
    return;
}
