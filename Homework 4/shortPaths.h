// Peter Wallace EC504 Spring 2021 Software HW 4
// Produced in collaboration with Jean Marc Achkar (jmachkar@bu.edu)
//


#ifndef SHORTPATH_H_
#define SHORTPATH_H_

#include <queue>
#include "myHeap.full.h"  // This is my version of HW 2.  You can include your own binary heap

#define LARGE1 9999999

using namespace std;


struct arc{
  struct arc *next;
  int length;
  int end;
  };

typedef struct node{
   struct arc *first; /* first arc in linked list */
   int id;  // The number of the vertex in this node 
   int key;  /* Distance estimate, named key to reuse heap code*/
   int P;  /* Predecessor node in shortest path */
   int position;  /* Position of node in heap, from 0 to Nm, where 0 is best */
   } nodeitem;

void BellmanFord(nodeitem N[], int Or, int Nm)
{
   // You program this, a  Bellman Ford algorithm that uses a work queue.  DO NOT implement this as 3 for loops. 
   // That can be very slow, and waste many iterations. 
   int i;
   N[Or].key = 0;
   N[Or].P = 0;
   queue<nodeitem*> bellman_queue;
   bool inqueue[Nm+1];
   for (int i = 0; i <= Nm; i++)
   {
       inqueue[i] = false;
   }
   //cout<<"After for"<<endl;
   inqueue[Or] = true;
   bellman_queue.push(&N[Or]);
   nodeitem *current;
   struct arc *edge;
   while (!bellman_queue.empty())
   {
      // cout<<"Start of while loop"<<endl;
       current = (bellman_queue.front());
       //cout<<"Dereference"<<endl;
       bellman_queue.pop();
       edge = current->first;
       inqueue[current->id] = false;
       while(edge != NULL)
       {
            //cout<<"Start of inner while"<<endl;
            if (N[edge->end].key > (current->key + edge->length))
            {
               N[edge->end].key = current->key + edge->length;
               N[edge->end].P = current->id;
                if (inqueue[edge->end] == false)
                {
                    bellman_queue.push(&N[edge->end]);
                    inqueue[edge->end] = true;
                }
            }
            edge = edge->next;
       }
   }
}/* end Bellman-Ford */
/* ---------------*/


void Dijkstra(nodeitem N[], int Or, int Nm)
{
    int Mark[Nm+1];
    struct arc *edge;
    int v, dv, min_d, min_v, finished;
    for (int i=1; i<=Nm; i++){
        Mark[i]=-1;
    }
    N[Or].key = 0;
    Mark[Or] = 1;
    finished = 1;
    min_v = Or;
    min_d = 0;
    while (finished < Nm){
        edge = N[min_v].first;  //update distances
        while (edge != NULL){ // explore the outgoing arcs of u 
            v = edge->end;
            dv = min_d + edge->length;
            if (N[v].key > dv){
                N[v].key = dv;
                N[v].P = min_v;
            }//if D > dv 
            edge = edge->next;
        }// while edge           
        
        min_d = LARGE1;
        for (int j = 0; j <= Nm; j++){
            if (Mark[j] < 1){
                if (N[j].key < min_d){
                    min_d = N[j].key;
                    min_v = j;
                }
            }
        } 
        Mark[min_v]=1;
        finished++;
    } 
} /* end Dijkstra */


void DijkstraHeap(nodeitem N[], int Or, int Nm)
{
   Heap<nodeitem> *thisHeap;
   thisHeap = new Heap<nodeitem>;
   struct arc *edge;
   nodeitem *node;

// set origin node's distance to 0
   N[Or].key = 0;

   // build priority queue (heap)
   for (int i = Or; i <= Nm; i++)
   {
       node = &N[i];
       thisHeap->insert(node);
   }

   // run Dijkstra's
   while (!thisHeap->IsEmpty())
   {
       node = thisHeap->remove_min();
       edge = node->first;
       while (edge != NULL)
       {
           if (N[edge->end].key > (node->key + edge->length))
           {
               // decrease key
               N[edge->end].P = node->id;
               N[edge->end].key = node->key + edge->length;
               thisHeap->decreaseKey(N[edge->end].position, N[edge->end].key);
           }
           edge = edge->next;
       }
   }




   
   // You write a Dijkstra algorithm using a binary heap; you can reuse the one from HW 2 with minor variations


} /* end DijkstraHeap */ 
#endif
